...
class AdminController extends MataModuleController {
    ...
    private function addUserToProjects($userId, $projectIds) {
        ...
        try {
            ...
            UserProject::model()->deleteAllByAttributes(array(
                "UserId" => $userId
            ));
            ...
            foreach ($projectIds as $projectId => $value) {
                if (isset($value) && $value == 1) {
                    ...
                    $newBinding = new UserProject();
                    $newBinding->attributes = array(
                        "UserId" => $userId,
                        "ProjectId" => $projectId
                    );

                    if ($newBinding->save() == false)
                        throw new CHttpException("Could not save new binding " . $newBinding->getFirstError());
                }
            }
