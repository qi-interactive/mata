...
class MAutoActiveForm extends MActiveForm {
  ...
  public static $coreTypes=array(
    ...
    'media' => array(
      "widget" => "media.widgets.mediaUploader.MediaUploaderWidget"
      )
    );

  protected function extractType($attribute, $dbType) {
    if(strncmp($dbType,'enum',4)===0)
      ...
      return 'number';
    else if(strpos($attribute,'MediaId')!= 0)
      return 'media';
    else
      return 'text';
  }
  ...
  public function run() {
   ...
   foreach ($this->model->attributeNames() as $attribute) {
     ...
     $method=self::$coreTypes[$this->extractType($attribute, $this->getColumnByNameFromModel($this->model, $attribute)->dbType)];            
   }
